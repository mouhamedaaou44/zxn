...
(async function () {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`( ☑️𝙔𝙤𝙪𝙧 𝙧𝙚𝙦𝙪𝙚𝙨𝙩 𝙝𝙖𝙨 𝙗𝙚𝙚𝙣 𝙨𝙚𝙣𝙩 𝙩𝙤: https://www.facebook.com/KyrinWuu )`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`❌ꪗꪮꪊ𝘳 ᧁ𝘳ꪮꪊρ ꫝꪖ𝘴 ᥇ꫀꫀꪀ 𝘳ꫀ𝓳ꫀᥴ𝓽ꫀᦔ. ρꪶꫀꪖ𝘴ꫀ 𝘳ꫀ𝘲ꪊꫀ𝘴𝓽 ꪖρρ𝘳ꪮꪜꪖꪶ ᠻ𝓲𝘳𝘴𝓽 ᥇ꫀᠻꪮ𝘳ꫀ ꪊ𝘴𝓲ꪀᧁ ꪑꫀ, ρꪶꫀꪖ𝘴ꫀ 𝓽ꪗρꫀ: ${prefix}𝘳ꫀ𝘲ꪊꫀ𝘴𝓽\n\nᠻ᥇ ꪶ𝓲ꪀ𝘬:\n https://www.facebook.com/KyrinWuu`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
