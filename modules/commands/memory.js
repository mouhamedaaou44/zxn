...
module.exports.run = async function({ api, event, args }) {
  ...
  const level = args[0];
  if(level < 1 || level > 20) return api.sendMessage("The level you choose must be within the range 1 -> 20", threadID, messageID);
 ...
 fs.writeFileSync(pathImg, imageBuffer);
  ...
  return api.sendMessage(
    { body: "You have 10 seconds to save the content below, then re-enter it!!", attachment: fs.createReadStream(pathImg) }, threadID, async (e, info) => {
