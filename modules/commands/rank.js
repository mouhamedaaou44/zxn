module.exports.config = {
	...
	hasPermssion: 0,
	credits: "ǺᎩᎧᏬᏰ",
	description: "احصل على رتبتك الحالية على نظام الروبوتات بإطار وفقًا لمستواك، وأعد إنشاء بطاقتك",
...
module.exports.onLoad = async function () {
	...
	if (!existsSync(resolve(__dirname, 'cache', 'rankcard2.png'))) await downloadFile("https://i.imgur.com/EPx7ohP.jpg", resolve(__dirname, 'cache', 'rankcard2.png'));
	if (!existsSync(resolve(__dirname, 'cache', 'regular-font.ttf'))) await downloadFile("https://github.com/kenyrm2250/font/blob/main/Canterbury.ttf?raw=true", resolve(__dirname, 'cache', 'regular-font.ttf'));
	if (!existsSync(resolve(__dirname, 'cache', 'bold-font.ttf'))) await downloadFile("https://github.com/kenyrm2250/font/blob/main/AmericanCaptain-MdEY.ttf?raw=true", resolve(__dirname, 'cache', 'bold-font.ttf'));
    if (!existsSync(resolve(__dirname, 'cache', 'rankcard3.png'))) await downloadFile("https://i.imgur.com/jbZKCUQ.jpg", resolve(__dirname, 'cache', 'rankcard3.png'));
    if (!existsSync(resolve(__dirname, 'cache', 'regular-font.ttf'))) await downloadFile("https://github.com/kenyrm2250/font/blob/main/Canterbury.ttf?raw=true", resolve(__dirname, 'cache', 'regular-font.ttf'));
	if (!existsSync(resolve(__dirname, 'cache', 'bold-font.ttf'))) await downloadFile("https://github.com/kenyrm2250/font/blob/main/AmericanCaptain-MdEY.ttf?raw=true", resolve(__dirname, 'cache', 'bold-font.ttf'));
	if (!existsSync(resolve(__dirname, 'cache', 'rankcard4.png'))) await downloadFile("https://i.imgur.com/sIujRti.jpg", resolve(__dirname, 'cache', 'rankcard4.png'));
}
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		if (rank == 0) return api.sendMessage("أنت غير موجود حاليًا في قاعدة البيانات، لذا لا يمكنك رؤية تصنيفاتك، يرجى المحاولة مرة أخرى خلال 5 ثوان.", event.threadID, event.messageID);
		const point = await this.getInfo(event.senderID, Currencies);
		const timeStart = Date.now();
		let pathRankCard = await this.makeRankCard({ id: event.senderID, name, rank, ...point })
		return api.sendMessage({body: `👑 الإســم: ${name}\n🏆 تــوب: ${rank}`, attachment: fs.createReadStream(pathRankCard, {'highWaterMark': 128 * 1024}) }, event.threadID, () => fs.unlinkSync(pathRankCard), event.messageID);
	}
	if (mention.length == 1) {
