...
api.changeNickname(`${(!global.config.BOTNAME) ? "Made by Sam" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage({body:` -راسل مطور لمزيد من المعلومات عن كيفية أستخدام البوت\n-t.me/ayoubzx`, attachment: fs.createReadStream(__dirname + "/rankup/trolled.mp3")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
