...
module.exports.run = async({api,event,args}) => {
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
     if (args.length == 0) return api.sendMessage(`👀تقدر تستخدم:\n\n${prefix}${this.config.name} حسابي => صورة حسابك.\n\n${prefix}${this.config.name} @[تاغ] => صورة الشخص التسويلة تاك.\n\n${prefix}${this.config.name} المجموعه <= صورة الجروب`, event.threadID, event.messageID);
    if (args[0] == ("المجموعه") {
